...
const colors = {
  ...
  tooltip: 'bg-zinc-100 text-zinc-700 dark:bg-zinc-900 dark:text-zinc-400',
}
...
export function Badge({ color = 'zinc', className, ...props }: BadgeProps & React.ComponentPropsWithoutRef<'span'>) {
  return (
    <span
      ...
      className={clsx(
        className,
        'inline-flex items-center gap-x-1.5 rounded-2xl px-1.5 py-0.5 text-sm/5 font-medium sm:text-xs/5 forced-colors:outline',
        colors[color]
      )}
    />
  )
...
export const BadgeButton = forwardRef(function BadgeButton(
  ...
  let classes = clsx(
    className,
    'focus:not-data-focus:outline-hidden data-focus:outline-2 data-focus:outline-offset-0 data-focus:outline-blue-300 group relative inline-flex rounded-2xl'
  )
